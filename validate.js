...
/**
 * Master validator method. Gets all answer groups and makes sure that each has
 * a selection before enabling the 'Submit/Continue' button.
 * 
 * @returns void
 */
function validate() {
 ...
 * 
 * @param radioGroup
 *            Array of radio buttons in the same group (same name)
...
function validateRadios(radioGroup) {
	for (var i = 0; i < radioGroup.length; i++) {
		...
		if (radioGroup[i].checked) {
			return true; // Question has been answered, return validity of
							// true
		}
	}
...
/**
 * Function not inherent to javascript, but would be useful. Returns Page
 * elements by tagname and typename.
 * 
 * @param tagname
 *            ex. form, table, p, input, etc.
 * @param typename
 *            ex. submit, radio, checkbox, etc.
 * @returns
 */
...
/**
 * Gets the radio buttons, separates then by name into arrays. Then the arrays
 * themselves are stored in another array.
 * 
 * @returns 2D Array radioGroups[i][j] where i represents the question, j
 *          represents the radio buttons representing the answers
 */
function getRadioGroups() {
	...
	for (var i = 0; i < radioNames.length; i++) {
		...
		// Only add the radio button group array to the parent if it is
		// populated
		if (rGroup.length > 0) {
...
/**
 * Adds an onclick listener to all radio buttons. Following this, Click any
 * radio buttons will force the validate().
 * 
 * @returns void
 */
...
// Page load instructions. Add validate to radio buttons only if radio button
// groups exist.
